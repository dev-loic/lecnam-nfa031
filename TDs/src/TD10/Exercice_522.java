...
public class Exercice_522 {
	...
	public static void main(String[] args) {
		...
		if(-1==result) System.out.println("Egalité parfaite, on ne peut rien prédire.");
		else if(0==result) System.out.println(" Le oui l'emporte !");
		else System.out.println(" Le non l'emporte !");
		
		// On verifie que les hommes et les femmes sont équitablement représentés
		equilibre=equilibreDesSexes(resultatsSondage);
		if (!equilibre) System.out.println("Un des deux sexes est moins représenté que l'autre :'(");
		else System.out.println("Ce vote est parfaitement égalitaire :)");
	...
	static int predictionResultat(int [][] tabSondage)
	{
		...
		if (nbOui>nbNon) result=0;
		else if (nbOui==nbNon) result = -1;
		else result = 1;
	...
	static boolean equilibreDesSexes(int [][] tabSondage)
	{
		...
		if (nbFemmes>nbHommes || nbHommes>nbFemmes) equilibre = false;
